...
// Program Description:
//
// This example demonstrates the available power modes on the EFM8UB1 STK using
...
void APP_enterPowerMode(uint8_t mode)
{
  ...
  switch (mode)
  {
    ...
    case SUSPEND_MODE:
      // Disable VMON0, disable LDO1 bias, and set LDO1 to suspend mode
      // to see datasheet current (VMON0 adds ~15 uA, LDO1 adds ~245 uA)
    ...
    case SHUTDOWN_MODE:
      // Disable VMON0 and LDO1 to see datasheet current (VMON0 adds ~15 uA, LDO1 adds ~255 uA)
      PWR_enterShutdown();
      break;
  }
}
