...
// 5) Move the SW104 switch to "USB" mode to connect VBUS to the VREGIN
//    pin, which is used to scale D+/D- voltage divider voltages relative
//    to VBUS (using VREGIN/4). The measured VBUS voltage will not be correct
//    if VREGIN is connected to VMCU/VBAT in "AEM" and "BAT" mode.
// 6) Connect the STK board to a USB host or charger via micro USB cable.
// 7) The LCD should display "SDP Detected", "DCP Detected", or one of the
//    proprietary charger types from charger_descriptions based on detected
//    charger type.
// 8) The LCD will also display the measured VBUS, D+, and D- voltages. VBUS
//    should measure between 4.4 to 5.25 V to meet USB specifications.
// 9) Disconnect the micro USB cable.
// 10) The LCD should go blank.
// 11) Connect the micro USB cable to another charger to detect a different
//     charger type.
//
// Target:         EFM8UB1
// Tool chain:     Generic
...
// efm8.Assert
// efm8.bsp.Efm8 Bsp
// efm8.bsp.Efm8 Memory Lcd
// efm8.PeripheralDrivers.Spi0
...
// Text row location (screen is 0 - top to 127 - bottom)
#define MESSAGE_DISP_ROW_1                78
#define MESSAGE_DISP_ROW_2                (MESSAGE_DISP_ROW_1 + FONT_HEIGHT)
#define MESSAGE_DISP_ROW_3                (MESSAGE_DISP_ROW_2 + FONT_HEIGHT)
#define MESSAGE_DISP_ROW_4                (MESSAGE_DISP_ROW_3 + FONT_HEIGHT)
...
void main(void)
{
  SI_SEGMENT_VARIABLE(str[22], uint8_t, RENDER_STR_SEG);
  bool runDemo = false;
  ...
  while (1)
  {
    ...
    if (chargerType <= MAX_CHARGER_TYPE)
    {
      ...
      DrawScreenText(charger_descriptions[chargerType], MESSAGE_DISP_ROW_1, 1);

      // Display VBUS voltage in mV
      sprintf(str, "VBUS: %u mV", vregin_mv);
      DrawScreenText(str, MESSAGE_DISP_ROW_2, 1);

      // Display D+ voltage in mV
      sprintf(str, "D+: %u mV", dp_mv);
      DrawScreenText(str, MESSAGE_DISP_ROW_3, 1);

      // Display D- voltage in mV
      sprintf(str, "D-: %u mV", dm_mv);
      DrawScreenText(str, MESSAGE_DISP_ROW_4, 1);
    }
    ...
    // Clear display text
    strcpy(str, "");
    DrawScreenText(str, MESSAGE_DISP_ROW_1, 1);
    DrawScreenText(str, MESSAGE_DISP_ROW_2, 1);
    DrawScreenText(str, MESSAGE_DISP_ROW_3, 1);
    DrawScreenText(str, MESSAGE_DISP_ROW_4, 1);
  }
