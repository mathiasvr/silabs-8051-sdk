...
// 2) Download code to a 'F560 device that is connected to a SMBus master.
// 3) Run the code.
//      a) The slave code will write data and read data from the
...
//      b) The test will indicate proper communication with the master by
   ...
   if (SMB0CN_ARBLOST == 0)
   {
      switch (SMB0CN & 0xF0)           // Decode the SMBus status vector
      {
         ...
         case  SMB_SRADD:
            ...
            {                          // match,
               SMB0CN_ACK = 0;         // ACK received address
            }
            break;
         ...
         // Slave Receiver: SMB0CN_STOp received while either a Slave Receiver or
         // Slave Transmitter
         case  SMB_SRSTO:
