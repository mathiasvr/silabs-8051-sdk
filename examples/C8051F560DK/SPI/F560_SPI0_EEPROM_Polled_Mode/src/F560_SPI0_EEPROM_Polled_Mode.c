...
//
// Method1:
// 1) Verify that J22 (Side A) is not populated.
// 2) Download the code to a 'F560 device that is connected as above.
...
// Method2 (optional):
// 1) Verify that J22 (Side A) is not populated.
// 2) Download code to a 'F560 device that is connected as above
...
// 5) HyperTerminal will print the progress of the write/read operation, and in
...
#define  EEPROM_CAPACITY   1024        // EEPROM capacity (bytes)
