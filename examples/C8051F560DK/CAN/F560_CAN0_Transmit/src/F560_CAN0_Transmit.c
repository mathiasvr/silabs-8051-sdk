...
// F560_CAN0_Receive.c.  The following two #defines should be set to the
...
//    connected as above to another device running the F560_CAN0_Receive
//    code.
//
...
void CAN0_TransferMO (uint8_t obj_num);
...
SI_UU32_t CAN_TX_COMPLETE;                  // Bit status register that is updated
...
SI_INTERRUPT(CAN0_ISR, CAN0_IRQn)
{
   ...
   if (status & TxOk)                  // If transmit completed successfully
   {
      ...
      if (Interrupt_ID <= 15)
      {
	      CAN_TX_COMPLETE.u32 |= (uint16_t) (0x01 << Interrupt_ID);
      }
      else if (Interrupt_ID <= 0x1F)
      {
         CAN_TX_COMPLETE.u16[MSB] |= (uint16_t) (0x01 << (Interrupt_ID - 16));
      }
   }
