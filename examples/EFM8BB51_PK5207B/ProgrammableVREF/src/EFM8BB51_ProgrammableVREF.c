...
// voltages: 1.2 V, 1.65V, 1.8V, and 2.4 V. The current VREF voltage can also
// be printed to a terminal window via the UART0 and a USB-to-UART bridge.
//
...
//   P0.4   - UART0 TX
//   P0.5   - UART0 RX
//   P0.0   - Board Controller enable - same pin as VREF; VCOM cannot be used
//   P2.0   - Display enable
//
//-----------------------------------------------------------------------------
...
// 4) Connect USB-to-UART bridge device's RX pin to UART0 TX (P0.4); This
//    project only transmits data, but bridge TX pin can also be connected to
//    UART0 RX if planning to add reception to firmware.
// 5) On the PC, open HyperTerminal (or any other terminal program) and connect
//    to the USB-to-UART Serial Port at 115200 baud rate and 8-N-1.
// 5) Run the code.
