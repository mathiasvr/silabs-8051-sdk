...
// Program Description:
//
// This program demonstrates how to configure the EFM8BB51 to use routines
// in UART1 peripheral driver (buffer configuration) to auto generate the
// baud rate and to write to and read from the UART1 interface. The program
// waits to see a sync word (0x55 or capital 'U') to synchronize the baud
// rates and then reads five lower case characters, capitalizes the letters,
// then outputs the capitalized letters.
//
// Resources:
//   SYSCLK - 24.5 MHz HFOSC0 / 1
//   UART1  - 115200 baud, 8-N-1
//   P0.0   - Board Controller (VCOM) enable (set high to enable board 
...
//    to the JLink CDC UART Port at any baud rate between 245000 and 374, and 8-N-1.
// 5) Run the code.
//    In Simplicity Studio IDE, select Run -> Resume from the menu bar,
//    click the Resume button in the quick menu, or press F8.
// 6) Type a capital 'U' to synchronize the baud rates. Note: any characters typed 
//    before the sync word (0x55) may result in garbage data afterwards
// 7) Type five lower case letters. Note: terminal will not display anything
...
void main (void)
{
   ...
   UART1_initWithAutobaud(UART1_BRPRESCALER_01, UART1_STOPLEN_SHORT, UART1_FEATURE_DISABLE,
                          UART1_PARITY_ODD, UART1_RX_ENABLE, UART1_RX_CROSSBAR, 
