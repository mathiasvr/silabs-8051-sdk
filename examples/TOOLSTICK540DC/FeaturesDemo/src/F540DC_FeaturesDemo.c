...
// F540 Daughter Card.
//
// Based on the setting of a edge-sensitive interrupt pin, the firmware
...
// defined by <BAUDRATE>.
//
// Pressing the switch on the daughter card toggles the LED between blinking
// or PWM mode.
//
// How To Test:
...
//	  - Moved INT0 to SW (P1.4)
//    - 8 Jun 2015
//
// Release 1.0 (GP) (25 MAR 2009)
//    -Initial Release
//
...
void ADC0_Init (void)
{
   ...
   REF0CN = 0x13;                      // Enable on-chip VREF and buffer
...
// SDCC does not include a definition for putchar() and so it is defined here.  T
// The prototype does not need to be explicitly defined because it is
// provided in stdio.h
...
void INT0_Init (void)
{
   ...
   IT01CF = IT01CF_IN0SL__P1_4;        // Configure /INT0 to P1.0
                                       // Active Low

   TCON_IT0    = 1;                         // /INT0 is edge sensitive (TCON)
...
SI_INTERRUPT(TIMER0_ISR, TIMER0_IRQn)
{
   ...
   SFRPAGE = LEGACY_PAGE;              // Configure for writes to SBUF0
...
SI_INTERRUPT(ADC0_ISR, ADC0EOC_IRQn)
{
   ...
   // The ADC, PCA, and TIMER registers are available on all the LEGACY_PAGE
...
{
   // The TMR3CN and Port registers are available on the LEGACY_PAGE, so there
