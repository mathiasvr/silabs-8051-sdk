//-----------------------------------------------------------------------------
// F540DC_ADC0_TemperatureSensor.c
//-----------------------------------------------------------------------------
...
// Program Description:
//
// This program measures the temperature sensor on an 'F540 device and outputs
...
// How To Test:
//
// 1) Connect the ToolStick540DC to a ToolStick Base Adapter.
...
// 4) Place a break-point inside the while(1) loop.
// 5) Observe the variable result.temperature. Check to make sure this is
//    close to the ambient temperature.
//
// ALTERNATE
// 1) Repeat steps 1-3 above.
// 2) Download and install ToolStick Terminal from the following url:
...
// 5) Check that the printed temperature is close to the ambient temperature
//
// Target:         ToolStick F540 DC
...
#include <SI_C8051F540_Register_Enums.h>
#include <stdio.h>
...
#define TEMP_SENSOR_GAIN     3330      // Temp Sensor Gain in (uV / degC)
                                       // Value obtained from datasheet

#define TEMP_SENSOR_OFFSET    856      // Temp Sensor Offset in mV
                                       // Value obtained from datasheet
...
void main (void)
{
   ...
   SFRPAGE = LEGACY_PAGE;              // Set for PCA0MD and SBUF0
   ...
   i = 1;
   while (i)                           // Simple loop waiting for device to heat
   {
      j = 1;
      while (j < 300) {j++;}
      i++;
...
temp_sense_out Get_Temperature_Voltage (void)
{
   ...
   // From Chapter 10 of the datasheet:

   // V[temp] = (Slope * TempC) + Offset; solving for TempC
   // TempC   = (V[temp] - Offset) / Slope

   // V(temp) = Accumulator / 2^16 * VREF (2.25V)
   // Offset  = TEMP_SENSOR_OFFSET (856 mV)
   // Slope   = TEMP_SENSOR_GAIN   (3.33 mV / deg C)

   // Putting in the numbers and simplifying the equation leads to:
   // TempC   = Accumulator * 661 / 65536 - 257

   result.temperature = (uint16_t) ((uint32_t)accumulator * (uint32_t)661 / (uint32_t)65536 - (uint32_t)257);

   // Note that the numbers were rounded when equation was solved.
   // This rounding does introduce error to the calculation, but it negligible
   // when looking for an estimated temperature.
