...
// F990 Daughter Card.
//
// Based on the setting of a port-match interrupt pin (S2, P0.2), the firmware
...
// 5) To toggle between blink and brightness modes, press switch S2 (P0.2).
// 6) Turn the potentiometer and see that the brightness of the LED changes.
//
//
// Target:         ToolStick990DC
...
SI_SBIT(RED_LED,    SFR_P0, 0);          // '0' means ON, '1' means OFF
SI_SBIT(YELLOW_LED, SFR_P0, 1);          // '0' means ON, '1' means OFF
SI_SBIT(SW2,        SFR_P0, 2);          // SW2 == 0 means switch pressed
SI_SBIT(R8_ENABLE,  SFR_P1, 5);          // Open-Drain output. Write '0' to
...
#define VDD             1875L          // Supply Voltage 
#define VREF            (1650L*2)      // Voltage Reference
#define ADC_COUNTS ((VDD*1024L)/VREF)  // Expected ADC code when potentiometer
...
void main (void)
{
   ...
   R8_ENABLE = 0;                      // Enable the Potentiometer
...
//-----------------------------------------------------------------------------
//
// PORT_MATCH_ISR is triggered upon a level transition on the P1.0 pin.
// If P1.0 is logic high, the firmware switches to a state that blinks the LED
// If P1.0 is logic low, the fimware applies PWM wave to the LED to control
// the brightness.
//
//-----------------------------------------------------------------------------
SI_INTERRUPT(PORT_MATCH_ISR, PMATCH_IRQn)
{
   ...
   else
   {
      ...
      XBR1   |= 0x01;                  // Put CEX0 on Crossbar (P1.6)

   }

   // Wait for the button to be released.
   while (SW2 == SWITCH_PRESSED);
...
SI_INTERRUPT(TIMER0_ISR, TIMER0_IRQn)
{
   ...
   if (overflows == OVERFLOWS_NEEDED)                    // Time to print
   {
      if ( (Blink_State == TIMER_MODE) &&  update)       // Timer mode
      {
         ...
         putchar (((Blink_Rate >> 1) / 10) + 0x30);      // Tens digit
         putchar (((Blink_Rate >> 1) % 10) + 0x30);      // Ones digit
         Print_String (" Hz\n");
...
SI_INTERRUPT(ADC0_ISR, ADC0EOC_IRQn)
{
   ...
   {
      if (ADC_result >= ADC_COUNTS)    // Assume max reading indicates VDD
      {                                // and not (VDD - 1 LSB)
...
// to P0.6.  Also enables the internal high-speed voltage reference.
//
//-----------------------------------------------------------------------------
void ADC0_Init (void)
{
...
// Parameters   : None
//
// Configure P0.2 (S2) to be a level-sensitive interrupt pin.
//
//-----------------------------------------------------------------------------
void PORT_MATCH_Init (void)
{

   P0MASK |= P0MASK_B2__COMPARED;      // Enable P0.2 to generate Port
                                       // Mis-Match events

   P0MAT |= P0MAT_B2__HIGH;            // Set expected state of P0.2.
                                       // P0.2 is now an active-high
                                       // interrupt pin.
