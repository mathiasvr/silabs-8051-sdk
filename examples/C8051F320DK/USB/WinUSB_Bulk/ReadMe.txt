...
This release contains the following components:
        ...
        * USB Bulk Driver (WinUSB)
        * USB Bulk Firmware Example
        * Host Application (SLAB_USB_Bulk.exe)
        * Host Application Source


Last updated: 05 NOV 2012

2.0 Running the Example on the C8051F320 target board
-----------------------------------------------------
	...
	1) Connect the hardware:
		...
		b) Install shorting blocks at the following positions: 
		   J3[5-6] (P2.2 LED) and J3[7-8] (P2.3 LED).
	...
	2) Build and download the Bulk Example firmware:
		...
		c) Connect to the target C8051F320/1 device (Alt+C, or IDE menu 
		   Debug->Connect).

		d) From the Project menu, select "Open Project" and browse to the 
		   Bulk Example Firware project "F320_USB0_Bulk.wsp", located by
           default in the "Silabs\MCU\Examples\C8051F320_1\USB_Bulk\Firmware"
           directory.
	...
	3) Run the Bulk Example firmware:
		...
		b) When Windows detects the device and the driver installation wizard opens:
		   ...
		   c. Browse to directory ..\Driver and select the file SLAB_USB_Bulk.inf,
              and click OK.
	...
	4) Run the Bulk Example host application:
		...
		a) Run the SLAB_USB_Bulk.exe application located by default in
		   C:\SiliconLabs\SimplicityStudio\v4\developer\sdks\8051\v#.#.#\examples\C8051F320DK\USB\WinUSB_Bulk\host
		   where #.#.# is the SDK version number.

		b) The USB Bulk Example application will display the current USB
		   devices connected to the PC. Select the device to be used for the 
		   example and open the device.

		c) Select a file to send by entering the file path into the File Path
           edit box or click the browse button to select a file.
           
        d) Press the Send button to transfer the selected file to the device
        
        e) Select a folder to receive the file by enetering the folder into
           the Folder Path edit box or click the browse button to select a
           folder.
           
        f) Press the Receive button to transfer a file from the device to the
           selected folder.
           
        g) LED1 indicates a read is in progress, LED2 indicates a write is in
           progress.
