...
// Program Description:
//
// Example software to demonstrate the C8051F32x SMBus interface in
...
//      - the 'F32x SMBus master should detect any conflict using SMB0CN_ARBLOST
//      - the 'F32x SMBus master should give up the bus, if necessary, and
//        switch to slave receiver mode
//      - the 'F32x SMBus master should transmit after the other master is done
// - Pinout:
//    P0.0 -> SDA (SMBus)
//    P0.1 -> SCL (SMBus)
//
...
// How To Test:
//
// 1) Download code to a 'F32x device that is connected to another C8051Fxxx
//    SMBus multimaster.
...
//
//      'F32x Multimaster                     Other C8051Fxxx Multimaster
//         LED (blinks) <------------------------------ Switch
//              SW --------------------------------> LED (blinks)
//
...
{
   static unsigned char count = 0;     // Count up to 128 Timer0 interrupts
