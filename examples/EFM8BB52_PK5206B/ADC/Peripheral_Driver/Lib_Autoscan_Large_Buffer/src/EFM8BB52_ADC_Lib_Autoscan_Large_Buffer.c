//-----------------------------------------------------------------------------
// EFM8BB52_ADC_Autoscan_Large_Buffer.c
//-----------------------------------------------------------------------------
...
//
// Timer 2 is set to trigger ADC conversions at 2048 Hz. Each conversion fills one
// place in the current buffer. After the buffer is full (1/64 second of samples),
...
// 6) After every press-release of push button 0, the program will output the
//    results of 2048 samples of the joystick.
//
// Target:         EFM8BB52
// Tool chain:     Generic
//
...
void main(void)
{
  ...
  while (1) {
    ...
    if (BUFFER_FULL) {
      FindMinMaxAverage();
      RETARGET_PRINTF("Samples taken: %u | Min (mV): %u | Max (mV): %u |"
...
void StartAutoscan(void)
{
  ...
  ADC0_enableAutoscan(&BUFFER[BUFFER_INDEX], SAMPLES_PER_SCAN,
                      ADC0_AUTOSCAN_MODE_CONTINUOUS);

  // The scan that will start after this one completes will begin
  //SAMPLES_PER_SCAN ahead  of the first.
