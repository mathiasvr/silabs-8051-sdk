...
// Program Description:
//
// Example software to demonstrate the C8051F336 SMBus interface in Slave mode
// - Hardware Acknowlege
...
// 2) Download code to a 'F336 device that is connected to a SMBus master.
...
uint8_t DATA_READY = 0;                     // Set to '1' by the SMBus ISR
                                       // when a new data byte has been
                                       // received.

#if(EXTHOLD_ENABLE)
uint8_t first_entry = 1;                    // Set to 1 to indicate that 
...
void SMBus_Init (void)
{
   SMB0CF = 0x01;                      // Use Timer1 overflows as SMBus clock
                                       // source;
                                       // Enable slave mode;
      ...
      switch (SMB0CN & 0xF0)           // Decode the SMBus status vector
      {
         ...
         case  SMB_SRADD:
            ...
            if((SMB0DAT & SMB0ADM & 0xFE) == (SMB0ADR & SMB0ADM & 0xFE))
            {
               ...
               {
                  SMB0CN_ACK = 1;                // Auto-SMB0CN_ACK the next master WRITE
               }
            } else 
            {
               SMB0CN_SI = 0;
         ...
         case  SMB_SRDB:
            ...
            if (rec_byte_counter < NUM_BYTES_WR)
            {
               ...
               SMB0CN_ACK = 1;                // Auto-SMB0CN_ACK next data byte
            ...
            {  
               ...
               SMB0CN_ACK = 0;                // Auto-NACK next data byte
            }
