...
SI_SEGMENT_VARIABLE(SW3_Cal_Value, uint16_t, code) = 0xFFFF;
...
{
   TMR2CN_TF2H = 0;                           // Clear Timer2 high-byte overflow flag
...
void Calibrate_SW3 (void)
{
   ...
   FLASH_ByteWrite ((uint16_t)(&SW3_Cal_Value)+MSB, SW3_T0_Count.u8[MSB]);
   FLASH_ByteWrite ((uint16_t)(&SW3_Cal_Value)+LSB, SW3_T0_Count.u8[LSB]);
...
// Return Value : None
// Parameters:
//   1) uint16_t addr - Flash address to write
//         Range - 0x0000 to 0x3DFF (15.5k user accessible flash)
//   2) uint8_t byte  - Byte to write
//         Range - 0x00 to 0xFF
//
