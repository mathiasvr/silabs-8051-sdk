//-----------------------------------------------------------------------------
// EFM8LB1_ADC_Autoscan_Multiple_Inputs.c
//-----------------------------------------------------------------------------
...
// ADC peripheral driver library, to sample two consecutive ADC inputs 32 times
// each without CPU intervention.
//
// The ADC's Autoscan feature can be used to scan multiple consecutive ADC channels
// without CPU intervention. In Autoscan mode, a conversion complete interrupt is only
// triggered after all conversions in the scan have been completed.
//
// Timer 3 is set to trigger ADC conversions at 0.5 Hz (every two seconds).
// Each conversion trigger causes the ADC to complete an entire scan. In this case,
// 64 conversions are completed, 32 from each of the two input channels. The two input
// channels are:
//    -P1.7 (Joystick)
//    -P2.1
//
// Inside the main loop, the CPU waits in Idle mode until an interrupt is triggered.
// This occurs once an autoscan is complete. After this, the results from the scan
// are printed out over UART.
//
//
// Resources:
...
#define START_CHANNEL (ADC0_POSITIVE_INPUT_P13)  // P1.7 (Joystick) is the ADC's
                                                  // first channel
...
//-----------------------------------------------------------------------------
#define BUFFER_SIZE (NUM_CHANNELS * NUM_SCANS) // The size, in 14-bit samples,
...
//    ADC0_POSITIVE_INPUT_P13,     //!< P1.7 (joystick) is ADC Positive Input
//    ADC0_POSITIVE_INPUT_P14,     //!< P2.1 is ADC Positive Input
//
...
SI_LOCATED_VARIABLE_NO_INIT(BUFFER[NUM_SCANS], scan_t, SI_SEG_XDATA, BUFFER_START_ADDR);
...
void main(void)
{
  ...
  ADC0_enableAutoscan((ADC0_AutoscanBuffer_t *)BUFFER, BUFFER_SIZE, ADC0_AUTOSCAN_MODE_CONTINUOUS);
