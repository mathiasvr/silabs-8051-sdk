...
// 5) Connect a USB cable from the USB port P5 on development board to a PC.
//    Specify the target baudrate in the constant <BAUDRATE>
// 6) On the PC, open Hyperterminal, or a similar program, and connect to the
//    target board's serial port using <BAUDRATE> and 8-N-1 (CP210x USB-to-UART
//    VCP drivers must be installed).
// 7) Compile and download code to C8051F500-TB (either device A or device B)
...
//    or pressing F8.
// 9) HyperTerminal will print the progress of the write/read operation,
//    and in the end will print the test result as pass or fail. Additionally,
//    The LED will blink fast during the write/read/verify operations.
//    If the verification passes, the LED will blink slowly. If it fails, the
//    LED will be OFF.
//
//
...
void main (void)
{
   ...
   SFRPAGE = LEGACY_PAGE;              // Set for printf()
...
void PORT_Init (void)
{
   ...
   P1SKIP   = 0x08;                    // Skip P1.3 on the crossbar

   // Enable SPI and UART0 on crossbar
