...
SI_SEGMENT_VARIABLE(scaleArray[MAX_NUM_ENTRY], uint8_t, SI_SEG_IDATA);
...
void DisplayLogger(uint8_t endLogNumToPrint)
{
    ...
    for(i = 0; i < sizeToPrint; i++)
    {
        ...
        case DISPLAY_ITEM_CELSIUS:
            ...
            RETARGET_SPRINTF(tempStr, "%d.%d C, %dm %ds",
                            (int16_t)(tempData / 10),
                            (int16_t)(tempData % 10),
                            (int16_t)(prevRecord.log.minutes),
                            (int16_t)(prevRecord.log.seconds));
            break;

        case DISPLAY_ITEM_FAHRENHEIT:
            tempData = CelsiusToFahrenheit(prevRecord.log.celsiusTemperature);
            RETARGET_SPRINTF(tempStr, "%d.%d F, %dm %ds",
                            (int16_t)(tempData / 10),
                            (int16_t)(tempData % 10),
                            (int16_t)(prevRecord.log.minutes),
                            (int16_t)(prevRecord.log.seconds));
        ...
        case DISPLAY_ITEM_HUMIDITY:
            RETARGET_SPRINTF(tempStr, "%d%%, %dm %ds",
                            (int16_t)(prevRecord.log.percentHumidity),
                            (int16_t)(prevRecord.log.minutes),
                            (int16_t)(prevRecord.log.seconds));
            break;
        }
...
void displayGraph(void)
{
    ...
    switch(displayItem)
    {
        case DISPLAY_ITEM_CELSIUS:
        {
            ...
            RETARGET_SPRINTF(str, "Celsius %d.%d C", tempData / 10, tempData % 10);
        }
        break;
        case DISPLAY_ITEM_FAHRENHEIT:
        {
            ...
            RETARGET_SPRINTF(str, "Fahrenheit %d.%d F", tempData / 10, tempData % 10);
        }
        break;
        case DISPLAY_ITEM_HUMIDITY:
        {
            ...
            RETARGET_SPRINTF(str, "Humidity %d%%", (uint16_t)(currentMeasurement.percentHumidity));
        }
        break;
    }
    ...
    RETARGET_SPRINTF(str, "%d", max_value);
    PrintString(str, 0, 8);

    RETARGET_SPRINTF(str, "%d", min_value);
    PrintString(str, 0, 112);

    // Displays the bottom axis to the LCD.
    // The posx constants passed to PrintString are so that the printed line is centered.
    #if MEASUREMENT_INTERVAL == HOURS
    RETARGET_SPRINTF(str, "1 sample per hour");

    #elif MEASUREMENT_INTERVAL == MINUTES
    RETARGET_SPRINTF(str, "1 sample per minute");

    #elif MEASUREMENT_INTERVAL == SECONDS
    RETARGET_SPRINTF(str, "1 sample per sec");

    #else // MEASUREMENT_INTERVAL == TWO_SECONDS
    RETARGET_SPRINTF(str, "1 sample two secs");
    #endif
...
void AddRecord (LOG_U* entry)
{
    ...
    for(i = 0; i < LOG_ENTRY_SIZE; i++)
    {
        ...
        FLASH_ByteWrite(nextAddr, entry->c[i]);
        nextAddr++;
    }
