...
// P0.0 - P0.5 delegated as port 0;
// P1.0 - P1.7 delegated as port 1;
// MCU  STK2011A  Function
// port port      description
// P0.6 EXP16     SMB SDA;
// P0.7 EXP15     SMB SCL;
// P2.3 EXP11     ADDR_EXT;
// P2.5 EXP13     INP_CHANGED_IRQ;
SI_SBIT(IOEXP_INP_CHANGED_IRQ, SFR_P2, 5);
SI_SBIT(IOEXP_ADDR_EXT, SFR_P2, 3);
#define P0DEVMASK                0x3F
#define P1DEVMASK                0xFF
#define UID_ST_ADDR 0x0FFF
#define UID_SP_ADDR 0x0FFC
...
#define EMAT_ENABLE(void) ( EIE2 |= EIE2_EMAT__ENABLED )         // Enable Port Match interrupts
#define EMAT_DISABLE(void) ( EIE2 &= ~EIE2_EMAT__ENABLED )       // Disable Port Match interrupts to prevent multiply interrupts
...
#define WRITE_DRIVE_CMD(port, data_out) {SFRPAGE = 0x0F;\
  switch (port) {\
    case 0: P0DRV = (data_out) & P0DEVMASK; break;\
    case 1: P1DRV = (data_out) & P1DEVMASK; break;\
  }SFRPAGE = LEGACY_PAGE;}
...
#define READ_DRIVE_CMD(port, retval) {SFRPAGE = 0x0F;\
  (retval) = ((port) == 0 ? (P0DRV & P0DEVMASK) : (P1DRV & P1DEVMASK));}
