...
// Voltage across the potentiometer is measured using the on-chip 8-bit ADC.
//
// The program also outputs the status of the LED to the UART at a baud rate
// defined by <BAUDRATE>.
//
// How To Test:
// 1) Connect the ToolStick300DC to a ToolStick Base Adapter.
...
// 5) To toggle between blink and brightness modes, press switch S1 (P0.3).
// 6) Turn the potentiometer and see that the brightness of the LED changes.
//
//      
// Target:         ToolStick300DC
...
SI_SBIT(S1, SFR_P0, 3);                   // SW = '0' means button is pressed
...
#define ADC_COUNTS      256            // Number of ADC counts

#define MAX_BLINK_RATE  50             // Max blink rate in Hz for Timer mode
...
//-----------------------------------------------------------------------------
//
// INT0_ISR is triggered upon a negative edge of P0.3 (S1 button press).
// If the device is currently blinking, the device is switched to a state that
// outputs PWM to control the brightness of the LED. If the device is in
// brightness mode, it switches to blink mode.
//
//-----------------------------------------------------------------------------
SI_INTERRUPT(INT0_ISR, 0)
{
   if (Blink_State == PWM_MODE)        // Currently in PWM mode
   {
      ...
      XBR1   &= ~0x40;                 // Remove CEX0 from Crossbar
   }
   else
   {
      ...
      // Switch To PWM Mode
      XBR0 = 0xC2;                     // Route PWM to P0.2
      XBR1   |= 0x40;                  // Put CEX0 on Crossbar (P1.3)
...
SI_INTERRUPT(ADC0_ISR, 8)
{
   ...
   {
      if (ADC_result == 0xFF)          // Assume max reading indicates VDD
      {                                // and not (VDD - 1 LSB)
   ...
   else
   {
      ...
      else
      {
         ...
         PWM_Duty_Cycle = (int16_t) ( ((int32_t)ADC_result * (int32_t)100) / 
                                     (int32_t)ADC_COUNTS );
      }
...
//
// P0.0   digital   open-drain    /INT0 input
// P0.1   digital   open-drain
// P0.2   digital   push-pull     LED (and CEX0 depending on Blink_State)
// P0.3   digital   open-drain    Switch
// P0.4   digital   push-pull     UART TX
// P0.5   digital   open-drain    UART RX
// P0.6   analog                  Potentiometer (ADC input)
//
//-----------------------------------------------------------------------------
void PORT_Init (void)
{
   P0MDIN    = 0xBF;                   // P0.6 is analog, rest of P1 digital
   P0MDOUT   = 0x14;                   // P0.4 is push-pull
...
void ADC0_Init (void)
{
   ...
   AMX0SL = 0xF6;                      // P0.6 is the positive input

   ADC0CF = (SYSCLK/ADC_CLOCK) << 3;   // ADC conversion clock <= 3MHz

   ADC0CF &= ~0x04;                    // Make ADC0 right-justified

   ADC0CF |= 0x01;                     // Set gain to 1.0

   EIE1  |= 0x04;                      // Enable ADC0 conversion-complete
                                       // interrupts
...
// Parameters   : None
//
// Configure /INT0 (P0.3, S1) to be an edge-sensitive interrupt pin.
//-----------------------------------------------------------------------------

void INT0_Init (void)
{
   //Configure INT0 to P0.3 (S1) and Active Low
   IT01CF    = IT01CF_IN0SL__P0_3 | IT01CF_IN0PL__ACTIVE_LOW;
