...
// Program Description:
//
// Example software to demonstrate the C8051F800 SMBus interface in
// Master mode.
...
//    - J3: P1.0_LED - P1.0
//    - J2
// 2) Ensure that J5 and J10 are not populated.
...
//            toggling P1.0_LED on and off each time a value is sent and received.
...
// TEST CODE-------------------------------------------------------------------
   ...
   while (1)
   {
      ...
      // Indicate that an error has occurred (GREEN_LED no longer lit)
      if (NUM_ERRORS > 0)
      {
         GREEN_LED = LED_OFF;
      }
...
//
// P1.6   digital   push-pull     GREEN_LED
//
// all other port pins unused
//
...
void Port_Init (void)
{
   ...
   P1MDOUT |= 0x01;                    // Make the GREEN_LED (P1.0) a 
                                       // push-pull output
