...
// 0 to Vref (1.8V) may be measured.  Although voltages up to Vdd may be
...
// 3) Compile and download code to the 'F70x device by selecting
//		Run -> Debug from the menus, clicking the Debug button in the quick menu,
//		or pressing F11.
// 4) Connect USB cable to P4.
// 5) On the PC, open HyperTerminal (or any other terminal program) and connect
//    to the COM port at <BAUDRATE> and 8-N-1
// 6) Connect a variable voltage source (between 0 and Vref) to the Port0 pins,
...
// Port1 ADC Inputs
uint8_t PIN_TABLE[ANALOG_INPUTS] = {0x08,0x09,0x0A,0x0B};
...
void main (void)
{
   ...
   while (1)
   {
      ...
      for(i = 0; i < ANALOG_INPUTS; i++)
      {
         ...
         measurement =  (RESULT[i] * 3360) / 1023;
         printf("P1.%bu voltage: %4ld mV\n",PIN_TABLE[i]%8,measurement);
      }
