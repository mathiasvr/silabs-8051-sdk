...
// --------------------
//
// This program accesses a SPI EEPROM using polled mode access. The 'F53xA MCU
...
// connections of the 'F53xA MCU are shown here:
//
...
//    the "A" SIDE.
// 5) (Optional for using "A" SIDE) Populated the UART TX/RX pin jumpers (HDR4).
//    Connect a USB cable from P1 on the target board to the PC.
//    On the PC, open HyperTerminal (or any other terminal program) and connect
//    to the COM port at <BAUDRATE> and 8-N-1. (CP210x USB-to-UART
//    VCP drivers must be installed).
// 6) Connect SPI bus to a Microchip 4 kB EEPROM 25LC320.
// 7) Run the code by selecting Run -> Resume from the menus,
//    or clicking the Resume button in the quick menu,
//    or pressing F8.
// 8) If the verification passes, the LED will blink slowly. If it fails,
//    the LED will be OFF.
//    If UART is connecting, HyperTerminal will print the progress of the
//    write/read operation, and in the end will print the test result as pass
//    or fail.
//
//
...
#define  F_SCK_MAX         1000000     // Max SCK freq (Hz)
#define  T_NSS_DISABLE_MIN 500         // Min NSS disable time (ns)
#define  EEPROM_CAPACITY   1248        // EEPROM capacity (bytes)
...
void PORT_Init (void)
{
   P0MDOUT  = 0x1D;                    // SPI pins: SCK, MOSI, NSS; UART TX are
                                       // push-pull
   P1MDOUT  = 0x08;                    // LED pin push-pull

   P1SKIP   = 0x08;                    // Skip the LED

   // Enable the UART and the SPI
