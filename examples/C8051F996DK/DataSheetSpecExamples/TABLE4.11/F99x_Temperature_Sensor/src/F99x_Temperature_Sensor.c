...
//
// This example demonstrates the Temperature Sensor's electrical
// characteristics as specified on table 4.11 of the datasheet.
//
// Refer to F99x_Temperature_Sensor_Readme.txt for more information 
...
// 7) Compile and download code to a 'F99x device on a C8051F996-TB development
...
#define AMBIENT_TC      26              // Ambient temperature
//-----------------------------------------------------------------------------
// Global Variables
//-----------------------------------------------------------------------------
...
void main (void)
{
    ...
    // Print opening text
    printf("F99x_Temperature_Sensor.c \n");
    printf("---------------------------------------------- \n");      
...
void UART_Output (uint8_t inputChar)
{
    ...
    // Vtemp = ADC * VREF / 1024 = ADC * 1.61 mV 
    //       = ADC * 16.1 /10
    //       = ADC * 16 / 10 + ADC * 1 / 100 
    
    Vtemp = ADCAverage * 8 / 5 ;
    Vtemp += ADCAverage / 100;
    switch (inputChar)
    {
        case '1':
