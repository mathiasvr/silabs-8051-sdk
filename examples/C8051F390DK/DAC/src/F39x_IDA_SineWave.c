...
// Program Description:
//
// This program outputs a sine wave using IDA0 and IDA1.  IDA0 and IDA1's 
// output is scheduled to update at a rate determined by the constant
...
// How To Test:
//
// 1) Run the code on a C8051F390 UDP MCU board
// 3) Place shorting block on J10 to place 1 kohm load on P0.1/IDA0
// 3) Place shorting block on J11 to place 1 kohm load on P1.2/IDA1
// 4) Use an oscilloscope to watch the IDA0 and IDA1 output on TB1
//    and verify that there is a sine wave with a peak amplitude of 2V
//
//
...
// P1.2     (skipped in crossbar)      IDA1 output pin
//-----------------------------------------------------------------------------
void PORT_Init (void)
{
   ...
   P1SKIP   = 0x04;                    // Skip P1.2 which is used for IDA1
   P1MDIN  &= ~0x04;                   // Configure P1.2 as an analog pin
...
void IDA_Init (void)
{
   ...
   SFRPAGE = IDAC1_PAGE;
   IDA1CN = 0xB2;                      // Enable IDAC1 for 2.0 mA full-scale
                                       // output; updated on Timer3 overflows
