...
// savings mode operation on the F39x/F37x. The MCU will sleep in low power
// idle mode with only the low frequency oscillator and timer T0 running. 
// When the MCU wakes up it will switch to a 24.5 MHz clock and turn on an 
// LED and wait 100 ms. The 100 ms delay also uses T0 and the low frequency
// oscillator but does not use idle mode or interrupts.
//
// How To Test:
//
// 1) Download code to a 'F390/F370 UDP MCU board
// 2) Place shorting block on J8: LED - P1.1.
// 3) Run the code.
// 4) The P1.1 LED should blink once a second.
//
//
...
SI_INTERRUPT_PROTO(T0_ISR, TIMER0_IRQn);
...
SI_INTERRUPT(T0_ISR, TIMER0_IRQn)
{
   OSCICN   =  0x83;                   // enable 24.5 MHz int osc
