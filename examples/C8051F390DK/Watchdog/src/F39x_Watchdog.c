...
//
// How to Test:
// 1) Compile and download code to a 'F39x/37x device.
...
#define SYSCLK       49000000 / 16     // SYSCLK frequency in Hz
...
// switch is pressed. When SW (P1.0) is pressed the code checks the RSTSRC
...
void main (void)
{
   ...
   SYSCLK_Init ();                     // Initialize system clock to 24.5/8 MHz
   ...
   while (1)
   {
    ...
    while(!SW);                 // Force the MCU to stay in this task as
                                    // long as SW is pressed. This task must
...
// Parameters   : None
//
// This routine initializes the system clock to use the internal 49 MHz / 16
