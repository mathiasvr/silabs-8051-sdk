//-----------------------------------------------------------------------------
// F502DC_ADC0_TemperatureSensor.c
//-----------------------------------------------------------------------------
...
// Program Description:
//
// This program measures the temperature sensor on an 'F502 device and outputs
...
// How To Test:
//
// 1) Change the #define AMBIENT to accurately reflect the
//    the temperature of the testing environment
// 2) Connect the ToolStick502DC to a ToolStick Base Adapter.
...
//    after a temperature measurement is taken (line 163).
// 5) Run the code by selecting Run -> Resume from the menus, clicking the
//    Resume button in the quick menu, or pressing F8. The program should halt
//    at the set break-point.
// 6) Check the variable "result.temperature" for the measured temperature. The
...
typedef struct {
   ...
   int32_t temperature;                    // Temp Sensor output in deg C
} temp_sense_out;
...
#define AMBIENT              2500      // Ambient temp in hundredths deg C
									   // (2500 = 25 degrees C)
...
void main (void)
{
   ...
   while (1)
   {
      ...
      putchar (((result.temperature / 1000) % 10)  + 0x30);  // Tens digit
      putchar (((result.temperature / 100) % 10)  + 0x30);   // Ones digit
      putchar ('.');
      putchar (((result.temperature / 10) % 10)  + 0x30);  // Tenths
      putchar ((result.temperature % 10)  + 0x30);         // Hundredths
...
temp_sense_out Get_Temperature_Voltage (void)
{
   ...
   result.temperature = (int32_t) (((uint32_t)accumulator * 100 * (uint32_t)1008 / (uint32_t)65536 - (uint32_t)414));
