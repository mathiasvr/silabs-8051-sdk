...
//
// How To Test:
// 1) Connect the ToolStick502DC to a ToolStick Base Adapter.
...
// 5) Press the switch SW to turn off the ADC and check that turning the
//    potentiometer has no effect on the blink rate.
// 6) Press the switch SW again to re-enable the ADC and potentiometer.
// 7) Turn the potentiometer and see that the brightness of the LED changes.
// 8) Press the switch SW to once again turn off the ADC.
// 9) Press the switch again to return to the original mode. Repeat steps
//    4-9.
//
//
...
//    - 26 AUG 2014
//
// Release 1.1 (GP) (06 AUG 2008)
//    -Fixed REF0CN initialization to set VREF to 2.25V
//
// Release 1.0
//    -Initial Revision (GP)
//    -17 JUL 2008
...
SI_SBIT(GPIO0, SFR_P1, 0);                // GPIO0/RTS - P0.6
...
void INT0_Init (void)
{
   ...
   IT01CF = 0x00;                      // Configure /INT0 to P1.0
                                       // Active Low
