...
// 8) Press PB1 to toggle oscilloscope mode between run (led on) and stop (led off)
// 9) Press PB0 to initiate a single sequence (led dim) (capture the first waveform
//    that triggers a rising or falling edge depending on menu configuration)
//    and stop.
// 10) When stopped (led off), pressing the joystick has the following effect:
...
//     Input - Analog input: joystick (P1.4) or external input (P0.2)
