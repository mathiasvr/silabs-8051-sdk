...
//
//-----------------------------------------------------------------------------
// How To Test: EFM8SB1 STK (SPI Master) + EFM8SB1 STK (SPI Slave)
//-----------------------------------------------------------------------------
// 1) Connect the first EFM8SB1 STK SPI pins to the second EFM8SB1 STK running
...
// 6) If the communication passes, the TEST_OUT pin on both the Master
//    and Slave boards will blink slowly. If it fails, the TEST_OUT pins will be low.
//
// Target:         EFM8SB1
// Tool chain:     Generic
//
...
void main(void)
{
   ...
   while (1)
   {
      // If no error has occurred, toggle the TEST_OUT pins on the Master and Slave
      // boards
      if (Error_Flag == 0)
      {
         TEST_OUT = 1;
