...
// Instruction Set
#define  SLAVE_TEST_OUT_HIGH   0x01    // Turn the Slave TEST_OUT signal high
#define  SLAVE_TEST_OUT_LOW    0x02    // Turn the Slave TEST_OUT signal low
	  ...
	  if (state == 0)
	  {
		 ...
		 switch (command)
		 {
			case SPI_READ_BUFFER:
			   ...
			   break;
			case SLAVE_TEST_OUT_HIGH:
			   TEST_OUT = 1;
			   state = 0;              // End of transfer (no bytes to send)

			   break;

			case SLAVE_TEST_OUT_LOW:
			   TEST_OUT = 0;
			   state = 0;              // End of transfer (no bytes to send)
