...
// Program Description:
//
// This example demonstrates the available power modes on the EFM8SB1 STK using
...
//    For all modes except Sleep, the display will show "*Active*" while 
//    in the mode. For Sleep mode the display is turned off.
// 8) Press PB0 (all modes except Stop) to return to the menu, or press
//    Reset (all modes) to restart the example. The LED turns on.
//
// Target:         EFM8SB1
// Tool chain:     Generic
//
...
#define SLEEP_MODE 4
#define LAST_POWER_MODE SLEEP_MODE
...
void main(void)
{
  ...
  // Clear the reset pin wake-up flag
  PWR_readAndClearWakeFlags();
  ...
  while (true)
  {
    ...
    // Turn off the display for Sleep Mode
    if (modeSelected == SLEEP_MODE)
    {
      UI_disableDisplay();
    }
...
void APP_enterPowerMode(uint8_t mode)
{
  // Clear the PMU wake flags for all modes to see lowest power
  ...
  switch (mode)
  {
    ...
    case IDLE_MODE:
      // Interrupts must enabled to return from Idle mode
      PWR_enterIdle();
      break;

    case STOP_MODE:
      // Disable VDDMON to see datasheet current (VDDMON adds ~10 uA)
      PWR_enterStop();
      break;

    case SUSPEND_MODE:
      // Disable VDDMON to see datasheet current (VDDMON adds ~10 uA)
      // Enter Suspend mode with port match as the wake source
      PWR_enterSuspend(PWR_PMAT_WF);
      wake_flags = PWR_readAndClearWakeFlags();
      break;

    case SLEEP_MODE:
      // Enter Sleep mode with port match as the wake source
