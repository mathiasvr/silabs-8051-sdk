...
// using the on-chip 10-bit ADC. Pressing the button S1 (P1.4) will switch
...
//
// How To Test:
// 1) Connect the ToolStick342DC to a ToolStick Base Adapter.
...
#define SYSTEMCLOCK     12000000L/4    // SYSCLK frequency in Hz
...
void TIMER3_Init (void)
{
   ...
   // Init reload values
   TMR3RL  = -(SYSTEMCLOCK / 12 / MIN_BLINK_RATE);  
...
void ADC0_Init (void)
{
   ...
   ADC0CF = (SYSTEMCLOCK/ADC_CLOCK) << 3;   
...
void LED_SwitchModes(void)
{
   ...
   else
   {
      ...
      XBR1   |= 0x01;                  // Put CEX0 on Crossbar (P2.2)
