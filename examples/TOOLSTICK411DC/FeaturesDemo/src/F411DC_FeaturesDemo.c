...
// Voltage across the potentiometer is measured using the on-chip 12-bit ADC.
//
// The program also outputs the status of the LED to the UART at a baud rate
// defined by <BAUDRATE>.
//
// How To Test:
// 1) Connect the ToolStick411DC to a ToolStick Base Adapter.
...
SI_SBIT(S1, SFR_P1, 4);                   // SW = '0' means button is pressed
//-----------------------------------------------------------------------------
// Global Constants
//-----------------------------------------------------------------------------
...
void main (void)
{
   ...
   RTC_Init ();                        // Configure RTC circuit
...
//-----------------------------------------------------------------------------
//
// PortMatch_ISR is triggered upon a button press on S1 (P1.4).
...
SI_INTERRUPT(ADC0_ISR, 10)
{
   ...
   if (Blink_State == TIMER_MODE)
   {
      ...
      {
         reload_speed =  ADC_result * BLINK_DIFF;
...
void OSCILLATOR_Init (void)
{
   ...
   OSCICN = 0x84;                      // Configure internal osc. to 24.5/8 MHz
...
// P2.1   digital   push-pull     LED (and CEX0 depending on Blink_State)
//
//-----------------------------------------------------------------------------
void PORT_Init (void)
{
   ...
   P2MDOUT   = 0x02;                   // P2.1 is push-pull

   P0SKIP    = 0xCF;                   // Skip all of P0 except for UART0 pins
   P1SKIP    = 0xFF;                   // Skip all pins on P1
   P2SKIP    = 0x01;                   // Skip to P2.1

   P0       |= 0x24;                   // Set port latch for pins RX and /INT0
                                       // to configure as inputs

   //Set up P1.4 (S1) for port match interrupts
   P1MAT |= P1MAT_B4__HIGH;
...
void TIMER3_Init (void)
{
   ...
   // Init reload values
   TMR3RL  = -(SYSCLK / 12 / MIN_BLINK_RATE);
...
void ADC0_Init (void)
{
   ...
   REF0CN = 0x0A;                      // Enable on-chip VREF = VDD

   ADC0MX = 0x0E;                      // P1.6 is the positive input
...
void RTC_Init (void)
{
   ...
   RTC0DAT = 0xC0;                     // enable the RTC, missing clock detect
}
...
// Parameters   : None
//
// Configure /INT0 (P0.2) to be a level-sensitive interrupt pin.
...
void INT0_Init (void)
{
   IT01CF = 0x02;                      // Configure /INT0 to P0.2
                                       // Active High
