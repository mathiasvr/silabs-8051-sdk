...
//-----------------------------------------------------------------------------
//
// Converts mV number to V string with units.
// e.g. 3300 to '3.300 V'
//
// mVolt - 0 to 3300 mV
// voltStr  - string with V info, length is 8 (added decimal point, space, and unit)
//
...
void drawScreen(uint32_t mVolt)
{
  ...
  {
	  convertMillivoltToVolt((uint16_t)mVolt, voltStr);
...
SI_INTERRUPT(PMATCH_ISR, PMATCH_IRQn)
{
  ...
  if (adcInput == ADCINPUT_JOYSTICK)
  {
    ADC0MX = ADC0MX_ADC0MX__ADC0P15; // P1.7
  }
  else if (adcInput == ADCINPUT_EXTERNAL)
  {
    ADC0MX = ADC0MX_ADC0MX__ADC0P9; // P1.1
  }
