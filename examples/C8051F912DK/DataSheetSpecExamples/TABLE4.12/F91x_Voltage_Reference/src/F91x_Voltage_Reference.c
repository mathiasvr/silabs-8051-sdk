...
// voltage of the ADC internal voltage reference.
//
// Refer to F91x_Voltage_Reference_Readme.txt for more information regarding
...
//    internal voltage reference in Table 4.12.
...
// 5) Remove all other shorting blocks on the board.
// 6) Default is High Speed Reference 1.65V enabled .
// 7) Record the current then push SW0.3 (P0.3) to disable Reference.
...
// 10) Record the current then push SW0.3 (P0.3) to disable Reference.
// 11) Read the new current to subtract previous current,
//     the difference is the Precision Reference current. 
// 12) If user want to measure Low Power mode, Set #define LOW_POWER_MODE 1 ,
//     return Step(4).This feature only available on ‘F912 and ‘F902 devices.
//
//
...
#if (LOW_POWER_MODE == 1)    
    SFRPAGE = CONFIG_PAGE;
    ADC0PWR = 0x80;                     // Set Low Power mode. 
    SFRPAGE = LEGACY_PAGE;
#endif    
}
