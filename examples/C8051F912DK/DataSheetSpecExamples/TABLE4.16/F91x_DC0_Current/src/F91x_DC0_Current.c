...
//
// Refer to F91x_DC0_Current_Readme.txt for more information regarding
// the functionality of this code example.
//
// How To Test (Table 4.16 - Bias Current):
// ---------------------------------------------------------
...
// 6) Download and run the code on a C8051F912-TB target board.
// 7) Record the current.then increase power supply output voltage to 3.3v,
...
void main (void)
{
...
#if (LOW_POWER == 1)
    DC0CF = 0x01;                       // Low Power mode enabled
#endif
    ...
    // Bypass enabled (auto switch), dc-dc oscillator disabled
    SFRPAGE = CONFIG_PAGE;
    DC0MD     = 0x04;
    SFRPAGE = LEGACY_PAGE;
