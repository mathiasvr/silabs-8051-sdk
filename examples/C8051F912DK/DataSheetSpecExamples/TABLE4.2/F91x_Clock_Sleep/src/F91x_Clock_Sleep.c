...
//
// Refer to F91x_Clock_Sleep_Readme.txt for more information regarding this
...
//    the Imeasure header pins.
// 3) Remove all other shorting blocks and select 2 Cell (3.3v) on SW4
// 4) Compile and download code to a 'F91x device on a C8051F912-TB development
...
//    The device should draw approximately .08 uA.
//
//
// Target:         C8051F91x
...
void Oscillator_Init (void)
{
    ...
    // Enlable VDD Supply monitor
    VDM0CN = 0x80;
    // waiting for VDD above the VDD warn minitor threshold
    while (!(VDM0CN & 0x20));
