...
#define SW11_SENSITIVITY   250         // Sensitivity value used for both
...
void main (void)
{
   ...
   while(1)
   {
      ...
      RED_LED = SW11_Status;           // switch variables
   }
...
{  
   ...
   CPT0MX = 0xC4;                      // Positive Mux: P2.0 - TouchSense Switch
   ...
   if(SW11_Timer_Count.u16 > (Get_Calibration() - SW11_SENSITIVITY))
   {
      SW11_Status = 0;
...
void Calibrate (void)
{
   ...
   if(switch_number == 0)
   {
      ...
      FLASH_ByteWrite ((uint16_t)(CAL_ADDRESS+MSB), SW10_Timer_Count.u8[MSB],
                       SCRATCHPAD);
      FLASH_ByteWrite ((uint16_t)(CAL_ADDRESS+LSB), SW10_Timer_Count.u8[LSB],
                       SCRATCHPAD);
   ...
   } else
   {
      ...
      // Write the expected switch value to the scratchpad
      FLASH_ByteWrite ((uint16_t)(CAL_ADDRESS+MSB), SW11_Timer_Count.u8[MSB],
                       SCRATCHPAD);
      FLASH_ByteWrite ((uint16_t)(CAL_ADDRESS+LSB), SW11_Timer_Count.u8[LSB],
                       SCRATCHPAD);
   }
...
void TouchSense_Init (void)
{
   ...
   CPT0MD = 0x8F;                      // Comparator interrupts disabled, 
                                       // lowest power mode
