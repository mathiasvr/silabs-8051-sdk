...
/// Joystick value
/// There is no joystick on BRD5002A, so this value will increase by one in
/// each USB transfer.
volatile uint8_t joystick_val;
...
void main(void)
{
  ...
  while (1)
  {
    ...
    inPacket[0] = btn0State;      // Send status of switch 0
    inPacket[1] = btn1State;      // and switch 1 to host
    inPacket[2] = (P0 & 0x0F);    // Port 0 [0-3]
    inPacket[3] = joystick_val;    // Joystick Value (BRD5002A has no joystick)
