...
//      or pressing F8.
// 7) Measure the supply current of Suspend mode using the multimeter.
//
//
//
// Target:         C8051F97x
...
void main (void)
{
   ...
   while(1)
   {
      CLKSEL &= ~CLKSEL_CLKDIV__FMASK; // Force clock divider to divide by 1

      // Wait for the divider to be applied
      ...
      for (i = 0; i < 25000; i++);     // Wait at least 15 us before
