...
//    (J16 for device A and J11 for device B).
//
// 2) Make sure the LIN jumpers in J14 (for A side) and J12 (for B side)
//    are connected.
//
...
//
// 3) Download the code to a F54x-TB (either device A or device B) that is
...
SI_SBIT(SW1, P1, 4);                     // SW1 = '0' means switch pressed
SI_SBIT(LED, P1, 3);                     // LED = '1' means LED on
...
#define  DTACK       0x10              // Slave Data SMB0CN_ACK
...
SI_INTERRUPT_PROTO(LIN0_ISR, INTERRUPT_LIN0);
