...
// phase accumulator, <phase_accumulator>, the upper bits of which are use d
// to access the sine lookup table.
//
// How To Test:
//
// 1) Connect the ToolStick362DC to a ToolStick Base Adapter.
...
// 4) Use an oscilloscope to watch the IDAC output on P0.1
//    and verify that there is a sine wave with frequency <FREQUENCY> and
//    with a peak amplitude of 2V
//
//
...
typedef union lng
{                                      // Access a int32_t variable as two
...
void main (void)
{
   ...
   TIMER3_Init (SYSCLK/SAMPLE_RATE_DAC);  // Use Timer3 as update scheduler
