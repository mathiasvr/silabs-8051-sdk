...
// consumption and clocking frequency on datasheet for the DC-DC in Table
// 4.14.
//
// Refer to F93x_DC0_Readme.txt for more information regarding
...
// 8) Measure the voltage on VDD/DC+ each time. The voltage should read between
//    the minimum and maximum values provided on the datasheet for the DC-DC 
//    Converter in Table 4.14
//
// How To Test (Table 4.14 - Output Current):
// ---------------------------------------------------------
// 1) Ensure a shorting block is placed the J11 (VBAT & WALL_PWR) .
// 2) Place VDD Select 1 CELL (1.8v) in the SW4 position and power the board
//    using the P2 power connector.
...
//    current that should read between the minimum and maximum values provided 
//    on the datasheet for the DC-DC Converter in Table 4.14
//
// How To Test (Table 4.14 - Output Power):
// ---------------------------------------------------------
//    Output Power = Output voltage * Output current.
// 1) The output power should be between the minimum and maximum values provided  
//    on the datasheet for the DC-DC Converter in Table 4.14
//
// How To Test (Table 4.14 - Clocking frequency):
// ---------------------------------------------------------
// 1) Ensure a shorting block is placed the J11 (VBAT & WALL_PWR) .
// 2) Place VDD Select 1 CELL (1.8v) in the SW4 position and power the board
//    using the P2 power connector.
// 3) Remove all of the shorting blocks on the board.
// 4) Default is internal oscillator, if user want to measure system clock source
//    ,set #define SYSTEM_CLOCK  1.
// 5) Download and run the code on the target board.
