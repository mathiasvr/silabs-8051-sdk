...
// the Analog to Digital Converter (ADC) in datasheet Table 4.9.
//
// Refer to F93x_ADC_Continuous_Sample_Readme.txt for more information
// regarding the functionality of this code example.
//
//
// How To Measure (Table 4.9 - ADC 8-bit Conversion Power Consumption):
// ----------------------------------------------------
//  1) Ensure that shorting blocks on a C8051F930-TB are attached to the
...
//  4) Set ADC0_MODE_SELECT to 0 to select ADC 8-bit conversion mode
//     with 300 ksps throughput.
...
//     listed on the datasheet for ADC0 Power supply current under Table 4.9.
//
//
// How To Measure (Table 4.9 - ADC 10-bit Conversion Power Consumption):
// ----------------------------------------------------
...
//  3) Remove all shorting blocks and attach an ammeter between
//     the J17 (Imeasure) headers.
//  4) Set ADC0_MODE_SELECT to 1 to select ADC 10-bit conversion mode with
//     300 ksps throughput.
//  5) Download and run the code on the target board.
//  6) Measure the current across Imeasure. This reading is the current with 
//     the ADC enabled.
...
//
//
// How To Measure (Table 4.9 - ADC 8-bit Tracking Power Consumption):
// ----------------------------------------------------
//  1) Ensure that shorting blocks are attached to the J11 (VBAT & WALL_PWR) 
//     and J8 (P0.2H) headers. 
//  2) Place VDD Select (SW4) in the 2 CELL position and power the board
...
//  3) Remove all shorting blocks and attach an ammeter between
//     the J17 (Imeasure) headers.
//  4) Set ADC0_MODE_SELECT to 2 to select ADC 8-bit Tracking mode with
//     200 ksps throughput.
//  5) Download and run the code on the target board.
//  6) Measure the current across Imeasure. This reading is the current with 
//     the ADC enabled.
...
//
//
// How To Measure (Table 4.9 - ADC 10-bit Tracking Power Consumption):
// ----------------------------------------------------
//  1) Ensure that shorting blocks are attached to the J11 (VBAT & WALL_PWR) 
//     and J8 (P0.2H) headers. 
//  2) Place VDD Select (SW4) in the 2 CELL position and power the board
...
//  3) Remove all shorting blocks and attach an ammeter between
//     the J17 (Imeasure) headers.
//  4) Set ADC0_MODE_SELECT to 3 to select ADC 10-bit Tracking mode with
//     260 ksps throughput.
//  5) Download and run the code on the target board.
//  6) Measure the current across Imeasure. This reading is the current with 
//     the ADC enabled.
...
//
//
// How To Measure (Table 4.9 - ADC Conversion Throughput Rate):
// -------------------------------------------------------------------
...
//     the sampling rate listed for ADC conversion throughput Rate 300 ksps
//     in Table 4.9.
//
//
...
// Available Modes:
// ----------------
// 0 - Power Consumption: Conversion 8 bit mode
// 1 - Power Consumption: Conversion 10 bit mode
// 2 - Power Consumption: Tracking 8 bit mode
// 3 - Power Consumption: Tracking 10 bit mode
//
...
void main (void)
{
    ...
    switch (ADC0_MODE_SELECT)
    {
        ...
        // 8 bit mode
        case 0:
            ADC0_Init (CONVERSION_MODE, ADC0_8_BIT);
            break;
        ...
        // 8 bit mode
        case 2:
            ADC0_Init (TRACKING_MODE, ADC0_8_BIT);
            break;
...
// either conversion or tracking mode. ADC0_Resolution can be either 8 bit or 
// 10 bit. The ADC interrupt is enabled if the user is testing the throughput
...
void ADC0_Init (uint8_t ADC0_Power_Mode, uint8_t ADC0_Resolution)
{
    ...
    if(ADC0_Power_Mode == CONVERSION_MODE)
    {
        ...
        // ADC0 8 bit resolution initialization settings
        if(ADC0_Resolution == ADC0_8_BIT)
        {
    ...
    // ADC0 TRACKING_MODE initialization settings
    else
    { 
        ...
        Timer2_Init (50000);            // Init Timer2 to generate overflows
                                        // to trigger ADC at 50kHz * 4 = 200kHz
    }
