...
//      - the 'F41x SMBus master should detect any conflict using SMB0CN_ARBLOST
//      - the 'F41x SMBus master should give up the bus, if necessary, and
//        switch to slave receiver mode
//      - the 'F41x SMBus master should transmit after the other master is done
// - Pinout:
//    P0.0 -> SDA (SMBus)
//    P0.1 -> SCL (SMBus)
//
...
// 3) Verify that MY_ADDR and MULTI_ADDR are defined to the proper values.
...
//    off each time SW3 is pressed, even if the buttons on both boards are
//    pressed at the same time.
//
//      'F41x Multimaster                     Other C8051Fxxx Multimaster
//         LED (blinks) <------------------------------ Switch
//              SW3 --------------------------------> LED (blinks)
//
...
SI_SBIT(SW, SFR_P1, 4);                   // Switch on P1.4
