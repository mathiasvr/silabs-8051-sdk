...
// using the on-chip 10-bit ADC.
//
// The program will switch between blinking the LED and outputting PWM to
// adjust its brightness occasionally, every 5 seconds by default.
//
// The program also outputs the status of the LED to the UART at a baud rate
// defined by <BAUDRATE>.
//
// How To Test:
// 1) Connect the ToolStick336DC to a ToolStick Base Adapter.
...
// 4) The device will switch between LED Blink / Brightness modes every 5
//    seconds.
// 5) When in LED Blink mode, turn the potentiometer and see that the blink
//    rate increases or decreases.
// 6) When in LED Brightness mode, turn the potentiometer and see that the
...
#define SECONDS_PER_MODE   5           // Seconds that the device will stay
                                       // in each LED mode (Blink/Brightness)
                                       // before switching

#define TIMER2_RELOAD_VALUE (SYSCLK / 12 / ADC_SAMPLE_RATE)
#define TIMER2_OVERFLOWS_PER_MODE (ADC_SAMPLE_RATE * SECONDS_PER_MODE)
...
void main (void)
{
   ...
   TIMER2_Init (TIMER2_RELOAD_VALUE);  // Init Timer2 for ADC
...
//-----------------------------------------------------------------------------
//
// This ISR is triggered upon a Timer2 overflow. This routine will periodically
// change the mode of the device between LED blink and LED brightness modes.
//-----------------------------------------------------------------------------
SI_INTERRUPT(TIMER2_ISR, TIMER2_IRQn)
{
   ...
   if (overflows >= TIMER2_OVERFLOWS_PER_MODE)
   {
      overflows = 0;
