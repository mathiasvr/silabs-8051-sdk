...
// CEX0 pin on on P1.0.  The PCA is configured to clock once every 10 us.
//
// Timer2 overflows are used to toggle pin P3.0 at an overflow rate of 4kHz.
// This signal at P3.0 is provided simply as a test method for the code.
//
...
// How To Test:
//
// 1) Connect P1.0 and P3.0 together.
...
// 6) Halt the program inside the PCA ISR.
// 7) Verify that the variable "capture_period" is equal to 50 (=500us).
//
// Alternatively:
// 1) Run a separate signal generator into the P1.0 pin.
// 2) Place the switch in "AEM" mode.
// 3) Connect the EFM8UB2 STK board to a PC using a mini USB cable.
// 4) Compile and download code to the EFM8UB2 STK board.
//    In Simplicity Studio IDE, select Run -> Debug from the menu bar,
...
// Global Variables
//-----------------------------------------------------------------------------
SI_SBIT(TEST_OUT, SFR_P3, 0);                  // Output for testing purposes
