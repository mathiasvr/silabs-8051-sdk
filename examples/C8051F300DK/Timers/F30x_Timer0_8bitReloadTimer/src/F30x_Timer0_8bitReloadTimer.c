...
// 8-bit counter/timer with reload mode. It uses the 'F300DK as HW platform.
// It uses the timer to create an interrupt at a certain rate and when the
// user's interrupt counter reaches the selected value the LED (P0.2)is toggled.
//
//
// Pinout:
//
...
// 7) Check that the LED is blinking (P0.2).
//
//
// Target:         C8051F300
...
// Parameters   : None
//
// This function configures the timer as a 8-bit timer with reload, interrupt
// enabled.
// Using the internal osc. at 24.5MHz with a prescaler of 1:8 and reloading
// TL0 register with TH0.
//
// Note: The timer uses a 1:48 prescaler.  If this setting changes, the
...
//-----------------------------------------------------------------------------
//
// Here we process the timer interrupt and toggle the LED when appropriate
//
//-----------------------------------------------------------------------------
