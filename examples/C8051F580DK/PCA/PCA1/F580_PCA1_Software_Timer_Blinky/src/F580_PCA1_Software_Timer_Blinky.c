...
// Program Description:
//
// This program uses the PCA1's Software Timer Mode to periodically schedule
// an interrupt.  Inside the ISR, a port pin is toggled to blink an LED on the
// target board.
//
// In this example, PCA1 Module 6 is used to generate the interrupt, and the
// port pin driving the LED is configured for push-pull mode.
//
// How To Test:
//
// 1) Download code to a 'F580 target board with the LED jumper ON.
...
#define PCA1_TIMEOUT ((SYSCLK/T0_CLOCKS)/LED_FREQUENCY/2)
...
void main (void)
{
   SFRPAGE = ACTIVE1_PAGE;             // Set SFR Page for PCA0 registers
...
// Parameters   : None
//
// This function configures the PCA1 time base to use T0 Overflows as a clock
...
// The code then sets up Software Timer mode for Module 6.
//
// On every interrupt from Module 6, software toggles the port I/O pin for the
...
//    associated module every time the PCA1 register increments and the new
//    16-bit PCA1 counter value matches the module's capture/compare
//    register (PCA1CPn).
//
...
void PCA1_Init (void)
{
   ...
   PCA1CPM6 = 0x49;                    // Module 6 = Software Timer Mode,
                                       // Enable Module 6 Interrupt flag,
                                       // Enable ECOM bit

   PCA1L = 0x00;                       // Reset PCA1 Counter Value to 0x0000
   PCA1H = 0x00;

   PCA1CPL6 = PCA1_TIMEOUT & 0x00FF;   // Set up first match
   PCA1CPH6 = (PCA1_TIMEOUT & 0xFF00) >> 8;

   // Set up the variable for the following match
   Next_Compare_Value = PCA1CP6 + PCA1_TIMEOUT;

   EIE2 |= 0x10;                       // Enable PCA1 interrupts

   PCA1CN_CR = 1;                      // Start PCA1
...
SI_INTERRUPT(PCA1_ISR, PCA1_IRQn)
{
   ...
   {
      PCA1CN &= ~0x86;                 // Clear other interrupt flags for PCA
   }
}
