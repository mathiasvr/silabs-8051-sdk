...
void main (void)
{
   ...
   while (1)
   {
      if (SW)
      {
         ...
         TMR4CAPH = (uint8_t)((T4_RELOAD_COUNT_UP >> 8) & 0x00FF);
         TMR4CAPL = (uint8_t)(T4_RELOAD_COUNT_UP & 0xFF);
      }
      else
      {
         // Load value to be a trigger when counting down from 0xFFFF
         TMR4CAPH = (uint8_t)((T4_RELOAD_COUNT_DOWN >> 8) & 0x00FF);
         TMR4CAPL = (uint8_t)(T4_RELOAD_COUNT_DOWN & 0xFF);
      }
...
// determine which direction TMR4CN_TMR4 will count.
//-----------------------------------------------------------------------------
void Timer4_Init (void)
{
   uint8_t SFRPAGE_save = SFRPAGE;          // Save Current SFR page
