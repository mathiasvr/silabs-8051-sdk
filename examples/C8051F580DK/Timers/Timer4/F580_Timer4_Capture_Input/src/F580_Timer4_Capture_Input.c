...
//
// A high-to-low transition on the T4EX pin will causes the 16 bit
// value in the associated timer (T4H, T4L) to be loaded into the
// capture registers (TMR4CAPH, TMR4CAPL). Whenever a capture occurs, the
// TMR4CN_EXF4 (TMR4CN.6) will become a logic high, and an interrupt will occur
...
SI_SBIT(SW, SFR_P1, 4);                  // 1 = not pressed, 0 = pressed
SI_SBIT(LED, SFR_P1, 3);                 // 1 = on, 0 = off
...
// transition on T4EX, the current value of TMR4CN_TMR4 will be captured in the
// TMR4CAP register.
//-----------------------------------------------------------------------------
void Timer4_Init (void)
{
...
// cycles in between them in the global variable, BUTTON_PRESS_DELAY.
//
//-----------------------------------------------------------------------------
SI_INTERRUPT(TIMER4_ISR, TIMER4_IRQn)
{
   static uint8_t button_state = 0;
   static SI_UU16_t timer4_a, timer4_b;
   static uint8_t t4_control;
