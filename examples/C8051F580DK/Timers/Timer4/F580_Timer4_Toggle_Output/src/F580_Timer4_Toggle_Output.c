...
#define TIMER4_RELOAD    (uint16_t)(0xFFFF - (SYSCLK/OUTPUT_FREQ/2))
...
// port pin associated with TMR2CN_T24 (P0.0). You can also monitor the state of
// the output by looking at TMR4CN_TOG4(TMR4CF.2).
//-----------------------------------------------------------------------------
void Timer4_Init (void)
{
   uint8_t SFRPAGE_save = SFRPAGE;          // Save Current SFR page
   ...
   // Load reload value
   TMR4CAPH = (uint8_t)((TIMER4_RELOAD >> 8) & 0x00FF);
   TMR4CAPL = (uint8_t)(TIMER4_RELOAD & 0xFF);
