...
// C8051F580 target board through the Local Interconnect Network (LIN).
// 
...
// Revision History:
//
// Release 1.2 / 03 AUG 2015 (SG)
//    - Revised for SimplicityStudio
//
// Release 1.1 / 11 MAR 2010 (GP)
//    -Tested with Raisonance
//
// Release 1.0 / 22 OCT 2008 (GP)
//    -Initial Revision
//
...
#include <SI_C8051f580_Register_Enums.h>
#include <stdio.h>
...
void LIN0_Master_Init (void)
{
   ...
   LIN0ADR = LIN0MUL;
   LIN0DAT = (uint8_t) ((LIN_PRESCALER << 6) | (LIN_MULTIPLIER << 1) 
               | (LIN_DIVIDER >> 8));
...
{
   static uint8_t sw1prev;                  // Status of the switch during the last
                                       // interrupt
   uint8_t SFRPAGE_save = SFRPAGE;
   SFRPAGE = ACTIVE1_PAGE;
   ...
   if (PCA0CN_CCF2)
   {
      ...
      if ((sw1prev == 0) && (SW1 == 1))
      {
         ...
         // Transmit at least one byte, even though slave does not need it
         // Prevents a timeout error on the slave
         LIN0ADR = LIN0SIZE;
...
{
   uint8_t ctrl, ID, command, status, error = 0;
