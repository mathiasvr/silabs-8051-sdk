...
#if (AUTO_BAUD == 0)
   // Load the multiplier, prescaler, and most significant bit of the divider.
   LIN0DAT = (uint8_t) ((LIN_PRESCALER << 6)  | (LIN_MULTIPLIER << 1) | (LIN_DIVIDER >> 8));   
#else
   // Load the prescaler and most significant bit of the divider
   LIN0DAT = (uint8_t) ((LIN_PRESCALER << 6) | (LIN_DIVIDER >> 8));
#endif
