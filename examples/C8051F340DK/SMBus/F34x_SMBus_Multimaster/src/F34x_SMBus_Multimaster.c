...
// Program Description:
//
// Example software to demonstrate the C8051F34x SMBus interface in
...
//      - the 'F34x SMBus master should detect any conflict using SMB0CN_ARBLOST
//      - the 'F34x SMBus master should give up the bus, if necessary, and
//        switch to slave receiver mode
//      - the 'F34x SMBus master should transmit after the other master is done
// - Pinout:
//    P0.0 -> SDA (SMBus)
//    P0.1 -> SCL (SMBus)
//
...
// 4) Connect the device to a to a SMBus - Multimaster device.
...
//
//      'F34x Multimaster                     Other C8051Fxxx Multimaster
//         LED (blinks) <------------------------------ Switch
//              SW --------------------------------> LED (blinks)
//
...
SI_SBIT(SW, SFR_P2, 0);                        // Switch on P2.0

SI_SBIT(LED, SFR_P2, 2);                      // LED on port P2.2
