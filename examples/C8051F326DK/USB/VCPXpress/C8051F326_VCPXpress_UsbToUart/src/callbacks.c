...
#include <SI_C8051F326_Register_Enums.h>    // SI_SFR declarations
#include "callback.h"
#include "buffers.h"
...
SI_SEGMENT_VARIABLE(rxCount, uint8_t, SI_SEG_IDATA); //tracks bytes REMAINING in the current uart rx buffer.
SI_SEGMENT_VARIABLE(txCount, uint8_t, SI_SEG_IDATA); //tracks the bytes REMAINING in the current uart tx buffer.
...
// to work properly.
SI_SEGMENT_VARIABLE_SEGMENT_POINTER(uartTxCurrent, uint8_t, SI_SEG_XDATA, SI_SEG_IDATA);// tracks current tx byte.
SI_SEGMENT_VARIABLE_SEGMENT_POINTER(uartRxCurrent, uint8_t, SI_SEG_XDATA, SI_SEG_IDATA);// tracks current rx byte.
