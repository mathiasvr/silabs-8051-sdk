...
#define MAX_SARCLK 25000000UL
#define MIN_SARCLK 12500000UL
                ...
                uint32_t sysclk)
{
  ...
  // Sysclk cannot be g+ater than the maximum
  SLAB_ASSERT(sysclk <= MAX_SYSCLK);

  // Sysclk cannot be lower than the minimum SARCLK
  SLAB_ASSERT(sysclk >= MIN_SARCLK)
  ...
  CLKGRP0 = (divider << CLKGRP0_CLKDIVSL__SHIFT) | CLKGRP0_CLKGRP0SL__SYSCLK;
  CLKGRP0 |= CLKGRP0_EN_SARCLK__ENABLE;

  // Set min tracking time to minimum needed during fastest adc clock (400ns)
  ADC0CF1 = 10 << ADC0CF1_ADTK__SHIFT;
                      	  	...
                      	  	uint8_t numChannels)
{
  ...
  SLAB_ASSERT(endChannel <= ADC0_POSITIVE_INPUT_VDD_DIV);
...
const ADC0_AutoscanBuffer_t * ADC0_getNextAutoscanBuffer(void)
{
  ...
  buffer = (ADC0_AutoscanBuffer_t *)(((ADC0ASAH & ADC0ASAH_STADDRH__FMASK) << 8)
  			    | (ADC0ASAL & ADC0ASAL_STADDRL__FMASK));
                      ...
                      uint8_t numElements)
{
  ...
  ADC0ASAH = (ADC0ASAH & ~ADC0ASAH_STADDRH__FMASK) 
			        | ((addr >> 8) & ADC0ASAH_STADDRH__FMASK);
  ADC0ASAL = (ADC0ASAL & ~ADC0ASAL_STADDRL__FMASK) 
			        | (addr & ADC0ASAL_STADDRL__FMASK);
